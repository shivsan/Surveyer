...
internal class SurveyAnswersControllerIntegrationTest {
    ...
    internal fun `Should create a survey response for a survey`() {
        every { surveyAnswersService.create(fakeSurveyAnswers) } returns fakeSurveyAnswers.copy(id = 1)

        mockMvc.perform(
            MockMvcRequestBuilders
                .post("/survey-answers")
                .content(objectMapper.writeValueAsBytes(fakeSurveyAnswers))
                .contentType(MediaType.APPLICATION_JSON)
        ).andExpect(MockMvcResultMatchers.status().isCreated)
            .andExpect(
                MockMvcResultMatchers.content().json(objectMapper.writeValueAsString(fakeSurveyAnswers.copy(id = 1)))
            )
    }
